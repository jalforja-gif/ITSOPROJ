...
async function loadPendingApplications() {
  try {
    ...
    if (data.length > 0) {
      data.forEach(row => {
        const suggested = row.suggested_classification ?? row.classification;
        const similarity = row.similarity_score ?? 'N/A';
        const readiness = row.readiness_score ?? 'N/A';
        newHTML += `
  <div class="table-row-card">
    ...
    <div data-label="Action">
      <button type="button" class="accept-btn"
        onclick='openApprovalModal(${row.ip_id}, "${suggested}", "${similarity}", "${readiness}")'>
        <ion-icon name="checkmark-circle-outline"></ion-icon>
      </button>
      ${row.remarks ? `<p class="remarks">⚠ ${row.remarks}</p>` : ''}
    </div>
  </div>
        `;
      });
...
setInterval(loadPendingApplications, 15000);
