...
<div class="bg-white rounded-lg shadow p-4 md:p-6 border overflow-hidden w-full">
  ...
  <!-- FORM START -->
  <form id="application-form-steps" action="add_entry.php" method="POST" enctype="multipart/form-data" class="space-y-4">
...
<div id="step-1" class="space-y-4 w-full"> 
  ...
  <div id="step-2" class="space-y-4 hidden">
    ...
    <div id="step-4" class="space-y-4 hidden">
      ...
      <p class="text-gray-500 text-xs md:text-sm">*Max size per file: 10MB.</p>
      <label class="block text-sm md:text-base font-medium mb-1">Application Form <i>(*pdf)</i></label>
      <input type="file" name="application_form" accept=".pdf" class="w-full border rounded px-3 py-2" required>
      <label class="block text-sm md:text-base font-medium mb-1">Project File <i>(*zip)</i></label>
      <input type="file" name="project_file" accept=".zip" class="w-full border rounded px-3 py-2" required>
      <label class="block text-sm md:text-base font-medium mb-1">Endorsement Letter <i>(*pdf)</i></label>
      <input type="file" name="endorsement_letter" accept=".pdf" class="w-full border rounded px-3 py-2" required>
      <label class="block text-sm md:text-base font-medium mb-1">
        Application Fee <span class="text-grat-500 text-xs md:text-sm"><i>*receipt from ipophil (optional)</i></span>
      </label>
      <input type="file" name="application_fee" class="w-full border rounded px-3 py-2">
      <label class="block text-sm md:text-base font-medium mb-1">
        Issued Certificate <span class="text-grat-500 text-xs md:text-sm"><i>*certificate from ipophil, if already issued (optional)</i></span>
      </label>
...
const fileRules = {
  ...
  endorsement_letter: ['pdf'],
  project_file: ['zip'],
  application_fee: ['pdf', 'jpg', 'jpeg', 'png'],
  issued_certificate: ['pdf']
};
...
document.querySelectorAll('input[type="file"]').forEach(input => {
  input.addEventListener('change', function () {
    ...
    const allowedExts = fileRules[fieldName];
    ...
    if (!allowedExts.includes(ext)) {
      Swal.fire({
        ...
        html: `
          <b>${file.name}</b><br>
          Allowed file type(s): <b>${allowedExts.join(', ')}</b>
        `
      });

      this.value = ''; // auto remove wrong file
    }
  });
});
</script>
