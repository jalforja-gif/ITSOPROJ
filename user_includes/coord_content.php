...
if (!empty($status_filter)) {
    ...
    if (!empty($where)) {
        $where .= " AND ip.status = 'For Revision'";
    } else {
        $where = "WHERE ip.status = 'For Revision'";
    }
    } else {
        if (!empty($where)) {
            $where .= " AND ip.status = ?";
    ...
    <tbody>
      ...
      <?php
        ...
        switch (strtolower($row['status'])) {
          ...
          case 'for revision': $status_class = 'status-revision'; break;
        }
      ?>
      ...
      <div class="modal-body">
        <div class="form-grid">
          <div class="form-group">
            ...
            <input type="file" name="endorsement_letter" class="file-input" accept=".pdf">
          </div>
          <div class="form-group">
            <label>Application Form</label>
            <div id="appform_name" class="file-name-display"></div>
            <input type="file" name="application_form" class="file-input" accept=".pdf">
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="file" name="project_file" class="file-input" accept=".pdf,.zip">
          </div>
...
function getFileLink(filename) {
  if (filename && filename !== 'X' && filename !== '0') {
    return `<a href="uploads/${filename}" target="_blank" style="color:#000;text-decoration:underline;">${filename}</a>`;
  } else {
...
// Safe attach: only if searchInput exists (index.php may include it)
...
// Form submit file size guard (attach only if updateForm exists)
...
// ------------- Close modals on outside click (consolidated) -------------
...
function updateStatuses() {
  fetch('user_includes/fetch_status.php')
    .then(res => res.json())
    .then(rows => {
      if (rows.error) return; // unauthorized
      rows.forEach(row => {
        ...
        if (tr) {
          ...
          if (badge) {
            ...
            // Reset classes
            badge.classList.remove('status-revision','status-pending','status-ongoing','status-completed');

            // Apply new class
            switch(row.status.toLowerCase()) {
              case 'for revision': badge.classList.add('status-revision'); break;
