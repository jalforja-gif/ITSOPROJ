...
// --- NEW: Call Python ML API to generate embedding ---
$escaped_title = escapeshellarg($title);  // escape for safety
$python_script = "C:\\xampp\\htdocs\\ITSO_System\\ml_api.py"; // adjust path
$command = "python $python_script $escaped_title";
$output = shell_exec($command);

// Convert Python JSON output to PHP array
$embedding = json_decode($output, true);
$embedding_json = json_encode($embedding); // store in DB as JSON

// Insert into database (now includes embedding)
$sql = "INSERT INTO intellectual_properties 
(title, authors, email, applicant_name, classification, endorsement_letter, status, application_form, submitted, application_fee, issued_certificate, project_file, date_submitted_to_itso, date_submitted_to_ipophil, expiration_date, department_id, campus_id, embedding, created_at) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
