...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // basic validation to avoid running query with empty/invalid id
    if (empty($ip_id) || !ctype_digit(strval($ip_id))) {
    ...
    try {
        ...
        if ($new_status === 'Completed') {
            ...
            ";
        } elseif ($new_status === 'For Revision') {
            $mail->Subject = "Your Intellectual Property Application Has Been For Revision";
            $mail->Body = "
                <p>Dear Applicant,</p>
                <p>We regret to inform you that your Intellectual Property application has been <strong>For Revision</strong>.</p>
                <ul>
                    <li><strong>Tracking ID:</strong> {$tracking_id}</li>
                    <li><strong>Title:</strong> {$title}</li>
                </ul>
