<?php
session_start();
error_reporting(E_ALL);
ini_set('display_errors', 1);

// Ensure ML API process is started (non-blocking)
include 'start_ml.php';

include 'db_connect.php';
date_default_timezone_set('Asia/Manila');

// -------------------- Collect form inputs --------------------
$title = $_POST['title'] ?? '';
$abstract = $_POST['abstract'] ?? '';  // NEW: abstract field
$authors = $_POST['authors'] ?? [];
if (!is_array($authors)) $authors = [$authors];
$authorsList = implode(", ", $authors);

$email = $_POST['email'] ?? '';
$applicant_name = $_POST['applicant_name'] ?? '';
$classification = $_POST['classification'] ?? '';
$status = $_POST['status'] ?? '';
$date_submitted_to_itso = $_POST['date_submitted_to_itso'] ?? null;
$submitted = isset($_POST['submitted_to_ipophil']) ? 1 : 0;
$date_submitted_to_ipophil = $_POST['date_submitted_to_ipophil'] ?? null;
$department_id = $_POST['department_id'] ?? '';
$campus_id = $_POST['campus_id'] ?? null;
$created_at = date('Y-m-d H:i:s');

if (!empty($date_submitted_to_itso)) {
    $date_submitted_to_itso .= ' ' . date('H:i:s');
} else {
    $date_submitted_to_itso = date('Y-m-d H:i:s');
}

// -------------------- Expiration calculation --------------------
function calculateExpiration($classification, $date_submitted_to_ipophil) {
    if (empty($date_submitted_to_ipophil)) return null;
    $years = [
        "Copyright" => 50,
        "Patent" => 20,
        "Trademark" => 10,
        "Utility Model" => 7,
        "Industrial Design" => 5
    ];
    return date('Y-m-d', strtotime("+{$years[$classification]} years", strtotime($date_submitted_to_ipophil)));
}
$expiration_date = calculateExpiration($classification, $date_submitted_to_ipophil);

// -------------------- File upload function --------------------
function uploadFile($fileInput) {
    if (!empty($_FILES[$fileInput]['name'])) {
        $targetDir = "uploads/";
        $fileName = basename($_FILES[$fileInput]["name"]);
        $targetFilePath = $targetDir . $fileName;
        if (move_uploaded_file($_FILES[$fileInput]["tmp_name"], $targetFilePath)) return $fileName;
        else echo "Failed to upload file: $fileName<br>";
    }
    return null;
}

$application_form    = uploadFile("application_form");
$endorsement_letter  = uploadFile("endorsement_letter");
$application_fee     = uploadFile("application_fee");
$issued_certificate  = uploadFile("issued_certificate");
$project_file        = uploadFile("project_file");

$conn->query("SET time_zone = '+08:00'");

// -------------------- Fetch existing abstracts for similarity check --------------------
$existing_abstracts = [];
$result = $conn->query("SELECT abstract FROM intellectual_properties");
while ($row = $result->fetch_assoc()) {
    if (!empty($row['abstract'])) {
        $existing_abstracts[] = $row['abstract'];
    }
}

// -------------------- Call ML API via cURL --------------------
$ml_api_url = "http://127.0.0.1:5000/predict";

$payload = json_encode([
    "title" => $title,
    "existing_abstracts" => $existing_abstracts,
    "new_abstract" => $abstract
]);

$ch = curl_init($ml_api_url);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
curl_setopt($ch, CURLOPT_POST, true);
curl_setopt($ch, CURLOPT_POSTFIELDS, $payload);

// execute request with a few retries in case API is still booting
$max_retries = 6;
$retry_delay = 1; // seconds
$response = false;
for ($attempt = 1; $attempt <= $max_retries; $attempt++) {
    $response = curl_exec($ch);
    if ($response !== false) {
        break;
    }
    error_log("add_entry1: ML API attempt $attempt failed: " . curl_error($ch) . "\n");
    sleep($retry_delay);
}

if ($response === false) {
    // final failure – log and continue with defaults
    error_log("add_entry1: ML API request ultimately failed after $max_retries attempts.\n");
    $ml_result = [];
} else {
    $ml_result = json_decode($response, true);
    if (!is_array($ml_result)) {
        error_log("ML API returned invalid JSON: " . substr($response, 0, 500));
        $ml_result = [];
    }
}
curl_close($ch);
if (!is_array($ml_result)) {
    // response wasn't valid JSON; log for debugging and fall back
    error_log("ML API returned invalid JSON: " . substr($response, 0, 500));
    $ml_result = [];
}

// -------------------- Extract ML results --------------------
$suggested_classification = $ml_result['suggested_classification'] ?? $classification;
// similarity_score and readiness_score default to 0.0 if missing
$similarity_score = isset($ml_result['similarity_score']) ? (float)$ml_result['similarity_score'] : 0.0;
$readiness_score = isset($ml_result['readiness_score']) ? (float)$ml_result['readiness_score'] : 0.0;
$embedding_json = json_encode($ml_result['embedding'] ?? []);

// -------------------- Insert into database --------------------
$sql = "INSERT INTO intellectual_properties 
(title, abstract, authors, email, applicant_name, classification, suggested_classification, similarity_score, readiness_score, endorsement_letter, status, application_form, submitted, application_fee, issued_certificate, project_file, date_submitted_to_itso, date_submitted_to_ipophil, expiration_date, department_id, campus_id, embedding, created_at) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

$stmt = $conn->prepare($sql);
if (!$stmt) die("Prepare failed: " . $conn->error);

// Correct bind_param types
$stmt->bind_param(
    "sssssssddsssisdssssisss",
    $title,
    $abstract,
    $authorsList,
    $email,
    $applicant_name,
    $classification,
    $suggested_classification,
    $similarity_score,
    $readiness_score,
    $endorsement_letter,
    $status,
    $application_form,
    $submitted,
    $application_fee,
    $issued_certificate,
    $project_file,
    $date_submitted_to_itso,
    $date_submitted_to_ipophil,
    $expiration_date,
    $department_id,
    $campus_id,
    $embedding_json,
    $created_at
);

if ($stmt->execute()) {
    $lastId = $stmt->insert_id;
    $year = date("Y");
    $tracking_id = "ITSO-" . $year . "-" . str_pad($lastId, 3, "0", STR_PAD_LEFT);

    $updateSql = "UPDATE intellectual_properties SET tracking_id = ? WHERE ip_id = ?";
    $updateStmt = $conn->prepare($updateSql);
    $updateStmt->bind_param("si", $tracking_id, $lastId);
    $updateStmt->execute();
    $updateStmt->close();

    $_SESSION['tracking_id'] = $tracking_id;
    $_SESSION['success_message'] = "Successfully added! Tracking ID: $tracking_id";

    // -------------------- Similarity warning --------------------
    if ($similarity_score >= 0.8) {
        $percent = round($similarity_score * 100, 1);
        echo "<script src='https://cdn.jsdelivr.net/npm/sweetalert2@11'></script>";
        echo "<script>
            Swal.fire({
                icon: 'warning',
                title: 'Potential Duplicate Detected',
                html: 'Your submission is very similar to an existing IP.<br>Similarity Score: {$percent}%',
            });
        </script>";
    }

    // Redirect based on role
    if (isset($_SESSION['role']) && $_SESSION['role'] === 'Chairperson') {
        header("Location: admin_dashboard.php?page=create_application");
    } elseif (isset($_SESSION['role']) && $_SESSION['role'] === 'Coordinator') {
        header("Location: index.php?page=create_application");
    } else {
        header("Location: login.php");
    }
    exit;
} else {
    die("Execute failed: " . $stmt->error);
}

$stmt->close();
$conn->close();
?>